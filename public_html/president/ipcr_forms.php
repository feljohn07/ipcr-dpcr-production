...
if ($idnumber) {
    // Prepare the SQL statement to fetch user information including college
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Construct the full name with suffix if available
            $ipcrname = mb_strtoupper($row["firstname"]) . " " . mb_strtoupper($row["middlename"]) . " " . mb_strtoupper($row["lastname"]);
            if (!empty($row["suffix"])) {
                $ipcrname .= ", " . $row["suffix"];
            }
            ...
            $ipcrnosuffix = mb_strtoupper($row["firstname"]) . " " . mb_strtoupper($row["middlename"]) . " " . mb_strtoupper($row["lastname"]);
            $ipcrnosuffix = htmlspecialchars($ipcrnosuffix, ENT_QUOTES); // Sanitize
            
            // Get the user's college
            $user_college = htmlspecialchars($row["college"], ENT_QUOTES); // Sanitize
        } else {
...
if ($idnumber) {
    ...
    if ($stmt = $conn->prepare($sql_user_info)) {
        ...
        if ($result_user_info->num_rows > 0) {
            ...
            // Proceed only if user_position and designation are not empty
            if (!empty($user_position) && !empty($designation)) {

                // Priority: Handle 'Dean' designation first
                if (strcasecmp($designation, 'Dean') === 0) { // Case-insensitive comparison
                ...
                // Handle 'Instructor to Assistant Professors'
                elseif ($role === 'IPCR' && 
                ...
                // Handle 'Associate Professors to Professors'
                elseif ($role === 'IPCR' && 
                ...
                // Handle 'Faculty with Designation'
                elseif ($role === 'IPCR' && 
                        $designation !== 'None' && 
                        !preg_match('/^associate-professor-[1-4]$/', $user_position) && 
                        !preg_match('/^professor-[1-5]$/', $user_position) && 
                    ...
                    $position_type = 'Faculty with Designation';

                    $sql_rdm = "SELECT support, core, strategic FROM rdm WHERE position = ?";
                    if ($stmt_rdm = $conn->prepare($sql_rdm)) {
                        $stmt_rdm->bind_param("s", $position_type);
                        $stmt_rdm->execute();
                        $result_rdm = $stmt_rdm->get_result();
                        if ($result_rdm->num_rows > 0) {
                            $row = $result_rdm->fetch_assoc();
...
// Fetch parameters from the GET request
// Fetch parameters from the GET request
$id_of_semester = isset($_GET['id_of_semester']) ? htmlspecialchars($_GET['id_of_semester']) : null; // Ensure you have the semester ID
$idnumber = isset($_GET['idnumber']) ? htmlspecialchars($_GET['idnumber']) : null; // Fetch user ID from GET request

$current_user_college = null; // Initialize the variable to hold the college value

// Check if both semester and user ID exist
if ($id_of_semester && $idnumber) {
    // Fetch the college for the given idnumber
    $college_sql = "SELECT college FROM usersinfo WHERE idnumber = ?";
    $college_stmt = $conn->prepare($college_sql);
    $college_stmt->bind_param("s", $idnumber);
    $college_stmt->execute();
    $college_stmt->bind_result($current_user_college);
    $college_stmt->fetch();
    $college_stmt->close();

    if (!$current_user_college) {
        die("No college found for the given ID number.");
    }

    // Fetch College Dean (Office Head) using the college obtained from the previous query
    ...
    $message_stmt->bind_param("ss", $idnumber, $id_of_semester); // Use $idnumber and $id_of_semester
    $message_stmt->execute();
    $message_stmt->bind_result($performance_message);
    $message_stmt->fetch();
    $message_stmt->close();
} else {
    // Handle case where semester ID or user ID is missing
    if (!$id_of_semester) {
        echo "Semester ID is missing.";
    }
    if (!$idnumber) {
...
// Check if the college dean query executed successfully
...
if ($collegeDean_result->num_rows > 0) {
    ...
    echo "No College Dean found for the specified college.";
}
...
if ($id_of_semester && $idnumber) {
    // Check if the user's ID number and semester ID exist in the to_ipcr_signature table
        $signatureCheckQuery = "SELECT idnumber, signature_created_at FROM to_ipcr_signature WHERE idnumber = ? AND semester_id = ?";
        $signatureCheckStmt = $conn->prepare($signatureCheckQuery);
        $signatureCheckStmt->bind_param("ss", $idnumber, $id_of_semester); // Use $idnumber here
        ...
        if ($signatureExists) {
            // Fetch the result
            $record = $signatureCheckResult->fetch_assoc(); // Fetch as an associative array
            $deans_first_signature = $record['signature_created_at']; // Get the signature_created_at value
            $fetched_idnumber = $record['idnumber']; // Get the idnumber value
            ...
            // Bind the results for the signature data
            $signature_stmt->bind_result($signature_data_office_head);
            $signature_stmt->fetch();
            $signature_stmt->close();
        } else {
            // If the record does not exist, set the signature data and created_at to null
            $signature_data_office_head = null;
            $deans_first_signature = null;
        }

// You can now use $deans_first_signature and $signature_data_office_head as needed
    ...
    $finalSignatureCheckStmt->bind_param("ss", $idnumber, $id_of_semester); // Use $idnumber here
    ...
    // Fetch College Dean's final signature only if the record exists
    if ($finalSignatureExists) {
        ...
        // If the record does not exist, set the final signature data to null
        $signature_data_office_head_final = null;
        $dean_final_sign_created_at = null; // Also set created_at to null
    }

    // Handle case where no College Dean was found for final signature
    if ($signature_data_office_head_final === null) {
        $collegeDeanFinal = "No College Dean found for final signature.";
    } else {
        // Process the signature data if found
        // (For example, you could display or process the signature data here)
    }
...
if ($idnumber && $id_of_semester) {
    ...
    $check_stmt->bind_param("ss", $idnumber, $id_of_semester);
    $check_stmt->execute();
...
///////////////////////////////////VPAAQA Signature for deans ipcr
$id_of_semester = isset($_GET['id_of_semester']) ? htmlspecialchars($_GET['id_of_semester']) : null; // Ensure you have the semester ID
$idnumber = isset($_GET['idnumber']) ? htmlspecialchars($_GET['idnumber']) : null; // Fetch user ID from GET request

if ($idnumber && $id_of_semester) {
    // Prepare a statement to check if there's a row in first_vpaa_to_ipcr_ofdeansignature
    $check_stmt = $conn->prepare("SELECT idnumber, signature_created_at FROM first_vpaa_to_ipcr_ofdeansignature WHERE idnumber = ? AND semester_id = ?");
    $check_stmt->bind_param("ss", $idnumber, $id_of_semester);
    $check_stmt->execute();
    $check_stmt->store_result();
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $signature_stmt->bind_param("s", $role);
        $signature_stmt->execute();
        $signature_stmt->bind_result($first_signature_to_deans_ipcr); // Renamed variable
        $signature_stmt->fetch();
        $signature_stmt->close();

        // Fetch the date created
        $check_stmt->bind_result($fetched_idnumber, $vpaa_first_sign_for_ipcr_created);
        $check_stmt->fetch(); // Fetch the result into variables
    }
...
function exportToWord() {
    ...
    <div class="container" style="padding: 0;"> 
        ...
        <p>
            ...
            <span style="text-decoration: underline; line-height: 1.2; font-weight: bold;"><?php echo htmlspecialchars($user_college); ?></span>
    ...
    <?php 
        if ($designation === 'Dean') {
            // When the designation is Dean
            if (!empty($first_signature_to_deans_ipcr)) { // Check if the variable is set and not empty
                ?>
                <img src="data:image/png;base64,<?php echo base64_encode($first_signature_to_deans_ipcr); ?>" alt="Signature"
            ...
            // When the designation is not Dean
            if (!empty($signature_data_office_head)) { // Check if the variable is set and not empty
                ?>
    ...
    ?>
        ...
        <?php if (isset($signature_first_president) && $signature_first_president): ?>
            <img src="data:image/png;base64,<?php echo base64_encode($signature_first_president); ?>" alt="Signature" 
    ...
    <td style="position: relative; padding: 0 0 0 0; height: 75px; border-bottom: none;">
                    <div style="position: absolute; bottom: 2px; width: 100%; text-align: center;">
                        <strong class="highlighted">  
                            <?php
                                // Check if the user's designation is "Dean" and adjust the output
                                echo ($designation === 'Dean') ? htmlspecialchars($vpaaqa) : htmlspecialchars($collegeDean);
                            ?>
                    </strong>
                    </div>
                </td>
                ...
                <?php 
                        // Determine which signature to use based on the designation
                        $signature_to_use = null; // Initialize to prevent undefined variable errors

                        if (isset($designation) && $designation === 'Dean') {
                            $signature_to_use = isset($vpaa_first_sign_for_ipcr_created) ? $vpaa_first_sign_for_ipcr_created : null;
                        } else {
                            $signature_to_use = isset($deans_first_signature) ? $deans_first_signature : null;
                        }

                        // Output the formatted date or a non-breaking space if the signature is empty
                        echo !empty($signature_to_use) ? date('m/d/Y', strtotime($signature_to_use)) : '&nbsp;';
                    ?>
                    </td>
                    <td style="position: relative; padding: 0 0 0 0; height: 75px; border-bottom: none;">
                    ...
                    <td rowspan="2" style="padding: 0; margin: 0; "> 
                        <?php 
                            echo !empty($president_first_sign_created) ? date('m/d/Y', strtotime($president_first_sign_created)) : '&nbsp;';
                        ?>
                    </td>
                </tr>
                <tr>
                ...
                <td class="for-signature" style="position: relative; padding: 0; height: 140px; width: 190px; overflow: hidden; border-bottom: none;">
                        <?php if ($final_signature_data && $final_signature_data): ?>
                            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; overflow: hidden;">
                                <img src="data:image/png;base64,<?php echo base64_encode($final_signature_data); ?>" alt="Signature" style="position: absolute; top: 50%; left: 50%; width: auto; height: auto; max-width: 100%; max-height: 100%; transform: translate(-50%, -50%); border: none; background-color: transparent;">
                            </div>
                        <?php else: ?>
                    ...
                    <td class="for-signature" style="position: relative; padding: 0; height: 140px; width: 190px; overflow: hidden; border-bottom: none; vertical-align: top; white-space: normal;">
                        ...
                        // Ensure variables are initialized
                            $signature_data_vpaaqa = $signature_data_vpaaqa ?? null; // Fallback to null if undefined
                            $signature_data_office_head_final = $signature_data_office_head_final ?? null; // Fallback to null if undefined

                            // Determine which signature data to use
                            if ($designation === 'Dean') {
                                $signature_to_display = $signature_data_vpaaqa;
                            } else {
                                $signature_to_display = $signature_data_office_head_final;
                            }
                        ?>

                        <?php if (!empty($signature_to_display)): ?>
                            <img src="data:image/png;base64,<?php echo base64_encode($signature_to_display); ?>" alt="Signature" style="position: absolute; top: 50%; left: 50%; width: auto; height: auto; max-width: 100%; max-height: 100%; transform: translate(-50%, -50%); z-index: -1;">
                        <?php else: ?>
                    ...
                    <td rowspan="3" style="padding: 0; margin: 0; vertical-align: top; text-align: center; width: 100px;"> <!-- Set vertical-align to top and text-align to center -->
                        ...
                        <div style="text-align: center; margin-top: 50%;"> <!-- Center the date value -->
                            <?php 
                                // Initialize created_at to null
                                $created_at = null;

                                // Check the user's designation
                                if ($designation === 'Dean') {
                                    // Use $vpaa_final_sign_created when the designation is Dean, or null if not set
                                    $created_at = $vpaa_final_sign_created ?? null; // Use null coalescing operator
                                } else {
                                    // Use $dean_final_sign_created_at when the designation is not Dean, or null if not set
                                    $created_at = $dean_final_sign_created_at ?? null; // Use null coalescing operator
                                }

                                // Check if created_at is not null or empty
                                if (!empty($created_at)) {
                                    // Create a DateTime object and format it
                                    echo htmlspecialchars((new DateTime($created_at))->format('m/d/Y'));
                                } else {
                                    echo ' '; // Display "null" when there is no data
                                }
                            ?>
                        </div>
                    </td>
                    ...
                    <?php if (isset($signature_data_vpaaqa) && $signature_data_vpaaqa): ?>
                                <img src="data:image/png;base64,<?php echo base64_encode($signature_data_vpaaqa); ?>" alt="Signature" style="position: absolute; top: 50%; left: 50%; width: auto; height: auto; max-width: 100%; max-height: 100%; transform: translate(-50%, -50%); z-index: -1;">
                            <?php else: ?>
                                <p style="text-align: center;"></p>
                            <?php endif; ?>
                    </td>
                    <td rowspan="3" style="padding: 0; margin: 0; vertical-align: top; text-align: center; width: 100px;"> <!-- Set vertical-align to top and text-align to center -->
                        ...
                        <td class="for-signature" style="position: relative; padding: 0; height: 140px; width: 190px; overflow: hidden; border-bottom: none;">
                                <?php if (isset($signature_final_data_president) && $signature_final_data_president): ?>
                                    <img src="data:image/png;base64,<?php echo base64_encode($signature_final_data_president); ?>" alt="Signature" style="position: absolute; top: 50%; left: 50%; width: auto; height: auto; max-width: 100%; max-height: 100%; transform: translate(-50%, -50%); z-index: -1;">
                                <?php else: ?>
                                    <p style="text-align: center;"></p>
                                <?php endif; ?>
                        </td>
