        ...
        if ($_FILES['file']['error'][$key] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file_tmp, $file_path)) {
              ...
              // Save file information to the database, including the semester_id
                // Save file information to the database, including the semester_id
                $insert_stmt = $conn->prepare("INSERT INTO task_attachments (task_id, id_of_task, task_type, file_name, user_idnumber, file_content, file_type, id_of_semester, firstname, lastname, upload_date, is_add) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)");
                $is_add = 'yes'; // Set the value for the is_add column

                // Update the bind_param call to match the number of parameters
                $insert_stmt->bind_param("iisssssssss", $task_id, $idoftask, $task_type, $file_name, $user_idnumber, $file_content, $file_type, $semester_id, $user_firstname, $user_lastname, $is_add);
                $insert_stmt->execute();
                $insert_stmt->close();
