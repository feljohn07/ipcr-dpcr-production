...
$current_user_firstname = $_SESSION['firstname'];
$current_user_lastname = $_SESSION['lastname'];

// Fetch approved tasks grouped by semester, including sibling_code
...
SELECT 
    ...
    st.start_date,
    st.end_date,
    COALESCE(stask.task_name, ctask.task_name, sptask.task_name) AS task_name,
    COALESCE(stask.description, ctask.description, sptask.description) AS description,
    ta.target,
    ta.num_file,  
    ta.status,
    ta.message,
    ta.deansmessage,  -- Fetching deansmessage from task_assignments
    ta.due_date,      -- Fetching due_date from task_assignments
    COALESCE(stask.sibling_code, ctask.sibling_code, sptask.sibling_code) AS sibling_code
FROM 
    task_assignments ta
LEFT JOIN 
...
WHERE 
    ta.assignuser = ? AND ta.status = 'approved' AND st.status = 'undone'
ORDER BY 
                        ...
                        <tr>
                            <td style="width: 100px; font-size: 10px;">
                                <form class="update-tasktype-form" id="update-task-form-<?php echo $task['id']; ?>">
                                    ...
                                    <input type="hidden" name="idnumber" value="<?php echo htmlspecialchars($current_user_idnumber); ?>"> <!-- Add this line -->
                                    ...
                                    <?php else: ?>
                                        <?php if ($documents_uploaded < $documents_req): // Only show the file upload form if not all documents are uploaded ?>
                                            <form id="upload-doc" enctype="multipart/form-data">
                                                ...
                                                <input type="hidden" name="semester_id" value="<?php echo $task['semester_id']; ?>">
                                                <input type="hidden" name="target" value="<?php echo $task['target']; ?>">
                                                <input type="hidden" name="task_name" value="<?php echo $task['task_name']; ?>">
                                                <input type="hidden" name="num_file" value="<?php echo $task['num_file']; ?>">
                                                <?php
                                                // Calculate the number of files remaining to be uploaded
                                                $remaining_files = $documents_req - $documents_uploaded;

                                                // Display file input fields for the remaining count
                                                for ($i = 0; $i < $remaining_files; $i++): ?>
                                                    <input type="file" name="file[]" accept=".doc, .docx, .pdf, .jpg, .jpeg, .png, .avi, .mov, .mkv">
                                                <?php endfor; ?>
                                                <button type="submit">Upload</button>
                                            </form>
                                        <?php else: ?>
                                            <p>All required documents have been uploaded.</p>
                                            <button type="button" onclick="showUploadInput(<?php echo $task['id']; ?>)">Add File</button>
                                            <div id="additional-file-upload-<?php echo $task['id']; ?>" style="display: none;">
                                                <form id="upload-additional-doc-<?php echo $task['id']; ?>" enctype="multipart/form-data">
                                                    <input type="hidden" name="task_id" value="<?php echo $task['id']; ?>">
                                                    <input type="hidden" name="task_type" value="<?php echo $task['task_type']; ?>">
                                                    <input type="hidden" name="semester_id" value="<?php echo $task['semester_id']; ?>">
                                                    <input type="file" name="file[]" accept=".doc, .docx, .pdf, .jpg, .jpeg, .png, .avi, .mov, .mkv" multiple>
                                                    <button type="submit">Upload Additional File</button>
                                                </form>
                                            </div>
                                        <?php endif; ?>
...
<script>
function showUploadInput(taskId) {
    var uploadDiv = document.getElementById('additional-file-upload-' + taskId);
...
document.addEventListener('submit', function(event) {
        // Ensure it matches the dynamically generated form ID
        if (event.target && event.target.matches('form[id^="upload-additional-doc-"]')) {
            ...
            xhr.open('POST', 'ipcrtaskspages/additional_upload_doc.php', true);
