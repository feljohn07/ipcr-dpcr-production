    ...
    foreach ($files['name'] as $key => $name) {
       // Replace all whitespace characters with a single underscore and remove parentheses
        ...
        if ($files['error'][$key] === UPLOAD_ERR_OK) {
            ...
            // Prepare SQL to insert the file details into the new table, including the 'is_add' column
            $stmt = $conn->prepare("INSERT INTO ipcr_file_submitted (user_id, firstname, lastname, college, group_task_id, task_id, semester_id, task_type, file_content, file_type, file_name, is_add) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

            // Add the value 'yes' for the 'is_add' column
            $stmt->bind_param("ssssssssssss", $idnumber, $firstname, $lastname, $college, $group_task_id, $task_id, $semester_id, $task_type, $fileContent, $fileType, $name, $is_add);

            // Set the value of 'is_add' to "yes"
            $is_add = 'yes';
