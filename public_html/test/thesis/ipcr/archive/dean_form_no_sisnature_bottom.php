...
$semester_id = isset($_GET['id_of_semester']) ? htmlspecialchars(string: $_GET['id_of_semester']) : null;
    ...
    if ($idnumber) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->fetch()) {
                ...
                $_SESSION['fullname'] = mb_strtoupper(string: "$firstname $middlename $lastname");
...
if ($idnumber !== null) {
    ...
    // Set signature_data to null regardless of fetched data
    $signature_data = null; // Override with null
}
...
if ($idnumber !== null && $semester_id !== null) {
    ...
    // Set final_signature_data and user_final_signature_created_at to null regardless of fetched data
    $final_signature_data = null; // Override with null
    $user_final_signature_created_at = null; // Override with null
}
...
if ($idnumber && $semester_id) {
    ...
    // Initialize variables to hold signature data
    $signature_data_vpaaqa = null;
    $vpaa_final_sign_created = null;
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        // Set the variables to null regardless of fetched data
        $signature_data_vpaaqa = null; // Override with null
        $vpaa_final_sign_created = null; // Override with null
    }
...
if ($idnumber && $semester_id) {
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $signature_stmt->bind_param("s", $role);
        $signature_stmt->execute();
        $signature_stmt->bind_result($signature_first_data_vpaaqa);
        $signature_stmt->fetch();
        $signature_stmt->close();

        // Fetch the date created
        $check_stmt->bind_result($fetched_idnumber, $vpaa_first_sign_created_to_dean);
        $check_stmt->fetch(); // Fetch the result into variables
    }
...
// Check if idnumber and semester_id are not null
if ($idnumber && $semester_id) {
    // Prepare a statement to check if there's a row in president_final_signature_to_ipcr
    ...
    // Initialize variables to hold signature data
    $signature_final_data_president = null;
    $president_final_sign_created = null;
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        // Set the variables to null regardless of fetched data
        $signature_final_data_president = null; // Override with null
        $president_final_sign_created = null; // Override with null
    }
...
function exportToWord() {
    ...
    <div class="container" style="padding: 0;"> 
        ...
        <p>
            I, <span style="text-decoration: underline; line-height: 1.2; font-weight: bold;"><?php echo htmlspecialchars($_SESSION['fullname']); ?></span> of the 
    ...
    <?php if (isset($signature_first_data_vpaaqa) && $signature_first_data_vpaaqa): ?>
            <img src="data:image/png;base64,<?php echo base64_encode($signature_first_data_vpaaqa); ?>" alt="Signature" 
    ...
    <tr>
                ...
                <td rowspan="2" style="padding: 0; margin: 0; "> 
                    <?php 
                        echo !empty($vpaa_first_sign_created_to_dean) 
                        ? (new DateTime($vpaa_first_sign_created_to_dean))->format('m/d/Y') 
                        : '&nbsp;'; // Use a non-breaking space to maintain width
                    ?>
                    </td>
                    <td style="position: relative; padding: 0 0 0 0; height: 75px; border-bottom: none;">
                    ...
                    <td rowspan="2" style="padding: 0; margin: 0; "> 
                        <?php 
                        echo !empty($president_first_sign_created) 
                        ? (new DateTime($president_first_sign_created))->format('m/d/Y') 
                        : '&nbsp;'; // Use a non-breaking space to maintain width
                        ?>
                    </td>
                </tr>
