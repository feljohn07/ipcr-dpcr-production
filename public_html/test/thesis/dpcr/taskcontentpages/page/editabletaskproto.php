...
<head>
    ...
    <style>
        ...
        .task-name, .description {
            width: 30%; /* Adjust width for Task Name and Description */
        }

        .due-date, .documents-required {
            width: 15%; /* Adjust width for Due Date and Documents Required */
        }
    ...
    <div class="container">
        ...
        <form method="post" action="../process/task_update.php" onsubmit="confirmUpdate(event)">
            ...
            <table>
                ...
                <tr>
                    <td><input type="text" name="semester_name" value="<?php echo htmlspecialchars($semester['semester_name']); ?>" required></td>
                    <td><input type="date" name="start_date" value="<?php echo htmlspecialchars($semester['start_date']); ?>" required></td>
                    <td><input type="date" name="end_date" value="<?php echo htmlspecialchars($semester['end_date']); ?>" required></td>
                    <td><input type="text" name="college" value="<?php echo htmlspecialchars($semester['college']); ?>" readonly></td>
                </tr>
            </table>

            <h3>Strategic Task Details</h3>
            <table id="strategic-tasks-container">
                <tr>
                    <th class="task-name">Task Name</th>
                    <th class="description">Description</th>
                    <th class=" due-date">Due Date</th>
                    <th class="documents-required">Documents Required</th>
                    <th>Action</th>
                </tr>
                <?php foreach ($strategic_tasks as $task): ?>
                <tr id="task-box-<?php echo $task['task_id']; ?>">
                    <td class="task-name"><textarea name="strategic_task_name[]" id="strategic_task_name_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $task_name = stripslashes($task['task_name']);
                        $task_name = str_replace('<break(+)line>', "\n", $task_name);
                        echo htmlspecialchars($task_name); 
                    ?></textarea></td>
                    <td class="description"><textarea name="strategic_description[]" id="strategic_description_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $description = stripslashes($task['description']);
                        $description = str_replace('<break(+)line>', "\n", $description);
                        echo htmlspecialchars($description); 
                    ?></textarea></td>
                    <td class="due-date"><input type="date" name="strategic_due_date[]" id="strategic_due_date_<?php echo $task['task_id']; ?>" value="<?php echo htmlspecialchars($task['due_date']); ?>" required></td>
                    <td class="documents-required"><input type="number" name="strategic_documents_required[]" id="strategic_documents_required_<?php echo $task['task_id']; ?>" value="<?php echo htmlspecialchars($task['documents_req']); ?>" min="0" required></td>
                    <td><button type="button" onclick="confirmDelete('<?php echo $task['task_id']; ?>', 'strategic')">Delete</button></td>
                </tr>
                <?php endforeach; ?>
            </table>
            ...
            <table id="core-tasks-container">
                ...
                <tr id="task-box-<?php echo $task['task_id']; ?>">
                    <td class="task-name"><textarea name="core_task_name[]" id="core_task_name_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $task_name = stripslashes($task['task_name']);
                        $task_name = str_replace('<break(+)line>', "\n", $task_name);
                        echo htmlspecialchars($task_name); 
                    ?></textarea></td>
                    <td class="description"><textarea name="core_description[]" id="core_description_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $description = stripslashes($task['description']);
                        $description = str_replace('<break(+)line>', "\n", $description);
                        echo htmlspecialchars($description); 
                    ?></textarea></td>
                    <td class="due-date"><input type="date" name="core_due_date[]" id="core_due_date_<?php echo $task['task_id']; ?>" value="<?php echo htmlspecialchars($task['due_date']); ?>" required></td>
                    <td class="documents-required"><input type="number" name="core_documents_required[]" id="core_documents_required_<?php echo $index; ?>" value="<?php echo htmlspecialchars($task['documents_req']); ?>" min="0" required></td>
                    <td><button type="button" onclick="confirmDelete('<?php echo $task['task_id']; ?>', 'core')">Delete</button></td>
                </tr>
                <?php endforeach; ?>
            </table>
            <button type="button" class="add-task-btn" onclick="addTask('core')">Add Core Task</button>

            <h3>Support Task Details</h3>
            <table id="support-tasks-container">
                <tr>
                    <th class="task-name">Task Name</th>
                    <th class="description">Description</th>
                    <th class="due-date">Due Date</th>
                ...
                <tr id="task-box-<?php echo $task['task_id']; ?>">
                    <td class="task-name"><textarea name="support _task_name[]" id="support_task_name_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $task_name = stripslashes($task['task_name']);
                        $task_name = str_replace('<break(+)line>', "\n", $task_name);
                        echo htmlspecialchars($task_name); 
                    ?></textarea></td>
                    <td class="description"><textarea name="support_description[]" id="support_description_<?php echo $task['task_id']; ?>" rows="4" required><?php 
                        $description = stripslashes($task['description']);
                        $description = str_replace('<break(+)line>', "\n", $description);
                        echo htmlspecialchars($description); 
                    ?></textarea></td>
                    <td class="due-date"><input type="date" name="support_due_date[]" id="support_due_date_<?php echo $task['task_id']; ?>" value="<?php echo htmlspecialchars($task['due_date']); ?>" required></td>
                    <td class="documents-required"><input type="number" name="support_documents_required[]" id="support_documents_required_<?php echo $index; ?>" value="<?php echo htmlspecialchars($task['documents_req']); ?>" min="0" required></td>
                    <td><button type="button" onclick="confirmDelete('<?php echo $task['task_id']; ?>', 'support')">Delete</button></td>
                </tr>
                <?php endforeach; ?>
            </table>
