...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $due_date = $_POST['due_date']; // Retrieve the due date from the POST data

    // Replace <break(+)line> with a real line break (new line character)
    $task_name = str_replace("<break(+)line>", "\n", $task_name);
    $task_description = str_replace("<break(+)line>", "\n", $task_description);
    ...
    if (!empty($selected_users)) {
        ...
        foreach ($selected_users as $user) {
            ...
            $target = $user['target'];
            $task_type = $user['taskType']; // This should hold the original task type
            ...
            $stmt = $conn->prepare("INSERT INTO task_assignments (idoftask, assignuser, lastname, firstname, task_type, status, message, created_at, semester_id, target, end_date, task_name, task_description, due_date) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?, ?)");
            $status = 'pending'; // Default status
            $message = ''; // Set message if needed

            if ($stmt) {
                // Bind the parameters with the updated task_name and task_description
                $stmt->bind_param("issssssisssss", $task_id, $user_idnumber, $lastname, $firstname, $task_type, $status, $message, $semester_id, $target, $end_date, $task_name, $task_description, $due_date);
