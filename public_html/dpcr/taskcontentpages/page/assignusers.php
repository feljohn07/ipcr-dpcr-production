...
<style>
    ...
    .modal-body thead {
        border: 1px solid #888;
        /* Add border to both header and body cells */
        position: sticky;
        top: 0;
        background-color: #fefefe;
    ...
    .modal-body th {
        ...
        /* Add a border to separate the header from the body */
    }
    ...
    .modal-content img {
        ...
        /* Adjust as needed */
        object-fit: cover;
        /* Ensures image covers the cell without distortion */
    }
    ...
    .status-container {
        ...
        /* Optional: Border around the status table */
    }
...
<body>
    ...
    <div class="header">
        <style>
            .header {
                margin-top: 60px;
                /* Adjust this value based on your navigation bar's height */
                padding: 20px;
                /* Optional: Add padding for spacing */
                background-color: #f8f9fa;
                /* Same as navigation background for continuity */
            }
        </style>
    ...
    <!-- Strategic Tasks -->
    <div class="tabledata">
        <h3 id="strategicTasks"
            style="background-color: #6c757d; color: white; padding: 10px 15px; border-radius: 5px; margin: 10px 0; font-size: 1.5em; font-weight: bold; text-align: left; transition: background-color 0.3s ease;">
            Strategic:
        </h3>
            ...
            <tbody>
                <?php foreach ($strategic_tasks as $task): ?>
                    <tr>
                        ...
                        <td style="padding: 8px; border: 1px solid #ddd;">
                            <?php echo htmlspecialchars(date('M/d/Y', strtotime($task['due_date']))); ?>
                        </td>
                        <td class="status-container" style="padding: 8px; border: 1px solid #ddd;">
                            <table style="width: 100%; border-collapse: collapse;">
                                <thead>
                                    <tr>
                                        <th style="padding: 5px; border: 1px solid #ddd; background-color: #ffeeba;">Pending
                                        </th>
                                        <th style="padding: 5px; border: 1px solid #ddd; background-color: #c3e6cb;">
                                            Approved</th>
                                        <th style="padding: 5px; border: 1px solid #ddd; background-color: #f5c6cb;">
                                            Declined</th>
                                    </tr>
                                </thead>
                                <tbody>
                        ...
                        <td style="text-align: center; padding: 8px; border: 1px solid #ddd;">
                            ...
                            '<?php echo htmlspecialchars($task['due_date']); ?>'
                                ...
                                <div class="spinner"
                                    style="display: none; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 20px; height: 20px; border: 2px solid rgba(0, 0, 0, 0.1); border-radius: 50%; border-top: 2px solid #3498db; animation: spin 1s linear infinite;">
                                </div>
                            </button>
            ...
            <div class="modal-body">
                <form id="assignForm">
                    <input type="hidden" id="task_id" name="task_id"
                        value="<?php echo htmlspecialchars($task['task_id']); ?>">
                    <input type="hidden" id="task_type" name="task_type"
                        value="<?php echo htmlspecialchars($task['tasktype']); ?>">
                    ...
                    <input type="hidden" id="task_description" name="task_description"
                    ...
                    <table style="border-collapse: collapse; width: 100%;">
                        <thead>
                            <tr style="border-bottom: 2px solid #ddd;">
                                ...
                                <th style="padding: 8px; border: 1px solid #ddd;" id="targetHeader">Target</th>
                                <!-- Dynamic header -->
                        ...
                        <tbody>
                            <?php foreach ($users as $user): ?>
                                <tr style="border-bottom: 1px solid #ddd;">
                                    ...
                                    <td style="padding: 8px; border: 1px solid #ddd;">
                                        <?php echo htmlspecialchars($user['idnumber']); ?>
                                    </td>
                                    <td style="padding: 8px; border: 1px solid #ddd;">
                                    ...
                                    <td style="padding: 8px; border: 1px solid #ddd;">
                                        <input type="number"
                                            name="target_<?php echo htmlspecialchars($user['idnumber']); ?>" min="0"
                                            step="1" placeholder="Enter target" class="target-input"
                                            data-id="<?php echo htmlspecialchars($user['idnumber']); ?>" disabled>
                                    </td>
                                    <td style="padding: 8px; border: 1px solid #ddd;">
    ...
    <div id="notification"
        style="display: none; padding: 10px; position: fixed; top: 10px; right: 10px; background-color: #28a745; color: white; z-index: 1000; border-radius: 5px;">
    </div>
    ...
    <div id="viewAssignedUsersModal" class="modal">
        ...
        ">
                <p>Target of Tasks/Total Target of each Users: (<span id="documentsRequired"></span>/<span
                        id="documentsRequiredByUser "></span>)</p>
            </div>
            ...
            setTimeout(() => {
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        // Select all elements whose id starts with "target_"
                        var targetElements = document.querySelectorAll('[id^="target_"]');

                        // Function to calculate the sum
                        function calculateTotalAssignedTarget() {
                            let sum = 0;

                            targetElements.forEach(function (input) {
                                sum += parseInt(input.value) || 0; // Parse value as an integer, default to 0 if empty or invalid
                            });
                        ...
                        // Loop through each element and get the value
                        targetElements.forEach(function (targetInput) {

                            targetInput.addEventListener('change', function (input) {

                                var targetMax = calculateTotalAssignedTarget();

                                // Prevent inputting more ten the documentRequired
                                if (targetMax > documentsReq) {
                                    targetInput.value--;
                                } else {
                                    document.getElementById('documentsRequiredByUser ').textContent = targetMax;

                                }

                                // // console.log(input + ' changed to: ' + targetInput.value);

                                // var noOfAssigned = document.getElementById('documentsRequiredByUser ').textContent;
                                // // update value or totalAssigned target

                                // // targetInput.max = documentsReq - documentsReqByUser;
                                targetInput.min = 0;
                            ...
                            // var value = targetInput.value; // Get the value of the input
                            // // console.log(value); // Output the value (or use it as needed)
                            // console.log(targetInput.max);
                            // targetInput.max = 1;
                            // targetInput.min = 0;

                            // for (let element of documentsRequiredElements) {
                            //     element.max = documentsReq - documentsReqByUser; // Set the max value
