...
$semester_stmt = $conn->prepare("SELECT * FROM semester_tasks WHERE office_head_id =  ? AND status = 'done' ORDER BY created_at DESC");
...
<head>
    ...
    <style>
        ...
        #semester-undone-content button {
            background-color: #4CAF50;
            color: #fff;
            padding: 10px 20px;
            border: none;
        ...
        #semester-undone-content button.cancel {
            background-color: #f44336;
        }

        #semester-undone-content button:hover {
    ...
    <script>
        ...
        function markSemesterUndone(semesterId) {
    // Show the modal for marking semester done
    const modal = document.getElementById('semester-undone');
    modal.style.display = 'block';

    const confirmButton = document.getElementById('confirm-semester-undone');
    confirmButton.onclick = function () {
        // Implement AJAX call to mark semester as done
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'taskcontentpages/process/mark_as_undone.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function () {
            if (xhr.status === 200) {
                // Store notification message in localStorage
                localStorage.setItem('notificationMessage', 'Semester marked as Undone successfully.');
...
<body>
            ...
            <?php foreach ($semesters as $index => $semester): ?>
                ...
                <div class="task-box">
                    ...
                    <div class="dropdown" id="dropdown-<?php echo htmlspecialchars($semester['semester_id']); ?>">
                        ...
                        <button class="mark-undone-button" onclick="markSemesterUndone(<?php echo htmlspecialchars($semester['semester_id']); ?>)">Mark As Undone</button>
                    </div>
                    ...
                    <h3 style="color : #00ff00;">
                        <?php 
                            if ($semester['status'] == "done") {
                                ...
                                echo "Status: " . htmlspecialchars($semester['semester_name']);
                            }
                        ?>
                    </h3>
                </div>
    ...
    <div id="semester-undone">
        <div id="semester-undone-content">
            <h3>Are you sure you want to mark this semester as Undone?</h3>
            <button id="confirm-semester-undone">Yes, Mark as Undone</button>
